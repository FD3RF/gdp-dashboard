...
def main():
    ...
    if run_btn or 'df' in st.session_state:
        ...
        # 最新信号
        st.subheader("🔔 最新信号")
        strategy = FourDimStrategy(st.session_state.get('config', FourDimConfig()))
        signal = strategy.get_latest_signal(df)
        
        if signal:
            direction_emoji = "🟢" if signal.direction == 'long' else "🔴"
            st.markdown(f"""
            <div style="background-color: {'#1b5e20' if signal.direction == 'long' else '#b71c1c'}; 
        ...
        with st.expander("📊 四维指标详情"):
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("成交量状态", signal.volume_signal if signal else "-")
            with col2:
                st.metric("MACD状态", signal.macd_signal if signal else "-")
            with col3:
                st.metric("SAR状态", signal.sar_signal if signal else "-")
            with col4:
                st.metric("支撑/阻力", signal.support_resistance if signal else "-")
